...
void SPI::init(int mosiPin, int misoPin, int clkPin, int csPin) {
	...
	bus_config.flags           = (SPICOMMON_BUSFLAG_SCLK | SPICOMMON_BUSFLAG_MOSI | SPICOMMON_BUSFLAG_MISO);
	bus_config.intr_flags      = ESP_INTR_FLAG_LEVEL1; //added to avoid error intr flag not allowed. see esp_intr_alloc.h

	ESP_LOGI(LOG_TAG, "... Initializing bus cc; host=%d", m_host);
